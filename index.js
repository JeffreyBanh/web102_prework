...
let totalFundedGames = GAMES_JSON.filter((games) => {
    return games.pledged >= games.goal;
}).length;
// create a string that explains the number of unfunded games using the ternary operator
const numberOfUnfundedGames = `A total of ${totalRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${totalUnfundedGames} remains unfunded. We need your help to fund these amazing games!`
// create a new DOM element containing the template string and append it to the description container
const newDOMElement = document.createElement('p');
newDOMElement.innerHTML = numberOfUnfundedGames;
...
let [firstGames, secondGame, ...others] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
const newTopElement = document.createElement('div');
newTopElement.innerHTML = firstGames.name
firstGameContainer.appendChild(newTopElement)
// do the same for the runner up item
const newTopElement2 = document.createElement('div');
newTopElement2.innerHTML = secondGame.name
secondGameContainer.appendChild(newTopElement2)